 ...
 * an optional dep. If `prom-client` is missing (e.g. on a stripped-down dev
 * machine), every recording method becomes a no-op and the /metrics endpoint
 * returns an empty registry — the rest of the app is unaffected.
 *
 * Counters / histograms (per Session 9 spec):
 *   - lira_bridge_state_transitions_total{from,to,source_type}
 *   - lira_bridge_webhook_processing_duration_seconds{provider,outcome}
 *   - lira_bridge_bridge_dispatch_latency_seconds
 *   - lira_bridge_outbound_api_calls_total{provider,status_code}
 *   - lira_bridge_refund_jobs_total{source,outcome}
 *   - lira_bridge_refund_latency_seconds{source}
 *   - lira_bridge_reserve_pool_usdc_units (gauge)
 */
@Injectable()
export class MetricsService implements OnModuleInit {
  ...
  private bridgeDispatchLatencyHist?: import('prom-client').Histogram<string>;
  ...
  private reservePoolGauge?: import('prom-client').Gauge<string>;
  ...
  onModuleInit(): void {
    ...
    this.stateTransitionsCtr = new this.prom.Counter({
      name: 'lira_bridge_state_transitions_total',
    ...
    this.webhookDurationHist = new this.prom.Histogram({
      name: 'lira_bridge_webhook_processing_duration_seconds',
      help: 'Duration of inbound webhook handling',
      labelNames: ['provider', 'outcome'],
      buckets: [0.01, 0.05, 0.1, 0.25, 0.5, 1, 2, 5, 10],
      registers: [this.registry],
    });
    this.bridgeDispatchLatencyHist = new this.prom.Histogram({
      name: 'lira_bridge_bridge_dispatch_latency_seconds',
      help: 'USDC_LOCKED → BRIDGE_DISPATCHED elapsed time',
      buckets: [0.05, 0.1, 0.25, 0.5, 1, 2, 5, 10, 30, 60],
      registers: [this.registry],
    });
    this.outboundApiCallsCtr = new this.prom.Counter({
      name: 'lira_bridge_outbound_api_calls_total',
      help: 'Outbound provider API call count',
      labelNames: ['provider', 'status_code'],
      registers: [this.registry],
    });
    this.refundJobsCtr = new this.prom.Counter({
      name: 'lira_bridge_refund_jobs_total',
      help: 'Refund job outcomes by path',
      labelNames: ['source', 'outcome'],
      registers: [this.registry],
    });
    this.refundLatencyHist = new this.prom.Histogram({
      name: 'lira_bridge_refund_latency_seconds',
      help: 'Refund executor elapsed time per job',
      labelNames: ['source'],
      buckets: [0.1, 0.25, 0.5, 1, 2, 5, 10, 30, 60, 120],
      registers: [this.registry],
    });
    this.reservePoolGauge = new this.prom.Gauge({
      name: 'lira_bridge_reserve_pool_usdc_units',
      help: 'Current Path-C reserve pool balance in USDC 6-decimal units',
      registers: [this.registry],
    });
    this.staleStateAlertsCtr = new this.prom.Counter({
      name: 'lira_bridge_stale_state_alerts_total',
  ...
  recordBridgeDispatchLatency(ms: number): void {
    this.bridgeDispatchLatencyHist?.observe(ms / 1000);
  }
  ...
  setReservePoolBalance(units: bigint): void {
    if (this.reservePoolGauge) {
      this.reservePoolGauge.set(Number(units));
    }
  }
